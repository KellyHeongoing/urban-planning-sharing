...
const projects = [
    {
        ...
        description: '探索未来城市发展模式，打造国际化商务中心。项目占地面积约117公顷，规划建设成为集总部办公、商业服务、文化娱乐为一体的综合性城市中心。',
...
function renderProjects() {
    const filteredProjects = projects.filter(project => {
        const matchesSearch = project.title.toLowerCase().includes(currentSearchTerm) ||
                             ...
                             project.tags.some(tag => tag.toLowerCase().includes(currentSearchTerm));
        const matchesCategory = currentCategory === '全部' || project.category === currentCategory;
        return matchesSearch && matchesCategory;
    });

    // 更新项目数量
    projectCount.textContent = `共找到 ${filteredProjects.length} 个项目`;
    ...
    if (filteredProjects.length === 0) {
        projectsGrid.innerHTML = `
            <div class="no-results" style="grid-column: 1 / -1; text-align: center; padding: 3rem; background: rgba(255, 255, 255, 0.1); border-radius: 16px; backdrop-filter: blur(10px);">
                <p style="font-size: 1.2rem; margin-bottom: 1rem; opacity: 0.8;">没有找到匹配的项目</p>
                <button onclick="clearFilters()" style="background: rgba(255, 255, 255, 0.2); color: white; border: 1px solid rgba(255, 255, 255, 0.3); padding: 0.75rem 1.5rem; border-radius: 8px; cursor: pointer; transition: all 0.3s ease;">
                    清除筛选条件
                </button>
            </div>
        `;
...
searchInput.addEventListener('focus', function() {
    this.placeholder = '输入关键词搜索项目... (Ctrl+K)';
});

searchInput.addEventListener('blur', function() {
    this.placeholder = '搜索项目...';
});
